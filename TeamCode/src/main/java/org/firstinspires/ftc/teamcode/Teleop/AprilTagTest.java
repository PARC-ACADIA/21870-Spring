 ...
 */
@TeleOp(name = "AprilTag Localization", group = "AprilTag")
public class AprilTagTest extends LinearOpMode {
    ...
    // 2. Instantiate and add your custom metadata directly as an object
    float tagPitch = -90.0f;
    float tagYaw = 0.0f;
    float tagRoll = 0.0f;
    AprilTagMetadata myCustomTag = new AprilTagMetadata(
            20,                                  // Tag ID
            "CustomTag",                        // Custom Name
            6.5,                                // Physical tag size (double)
            new VectorF(0.00f, 72.0f, 6.0f),  // Field positions (floats)
            DistanceUnit.INCH,                  // Distance Unit
            Quaternion.fromMatrix(
                    new Orientation(AxesReference.EXTRINSIC, AxesOrder.XYZ, AngleUnit.DEGREES, tagPitch, tagYaw, tagRoll, 0).getRotationMatrix(),
                    0
            )              // Orientation
    );
    ...
    private void initAprilTag() {
        myAprilTagLibraryBuilder.addTag(myCustomTag);
        AprilTagLibrary myCustomLibrary = myAprilTagLibraryBuilder.build();
     ...
     */
// 1. Create a clean builder instance
